...
// --- AKA transfer layer (replaces the SAMD21 DMA path) ---------------------
static uint16_t s_slice[Renderer::ScreenWidth * Renderer::SliceHeight];

Renderer::Renderer()  {}
Renderer::~Renderer() {}

// Slices are composed by the engine at META resolution, then handed to the
// shared presenter which assembles + scales them to the AKA screen.
void Renderer::draw(Game * engine) {
    const uint8_t slices = ScreenHeight / SliceHeight;
    for (uint8_t i = 0; i < slices; ++i) {
        uint8_t sliceY = i * SliceHeight;
        engine->drawOnSlice(sliceY, s_slice);
        gnw_present_slice_bswap(sliceY, SliceHeight, s_slice, ScreenWidth);   // donnees Steph en big-endian
    }
    gnw_present_flush();
}
